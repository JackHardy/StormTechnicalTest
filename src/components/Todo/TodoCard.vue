...
<template>
  <div
      :class="{
        'h-60': open,
        'h-fit': !open,
        'grayscale pointer-events-none': TodoListStore.editingIndex && TodoListStore.editingIndex !== cardIndex,
        'ring-brand-red ring ring-offset-2 shadow-2xl pointer-events-none': TodoListStore.editingIndex && TodoListStore.editingIndex === cardIndex,
      }"
        ...
        <div class="flex items-center gap-2">
          <div class="relative flex size-7">
            <div class="z-10 size-7 flex items-center rounded-full p-1 bg-brand-red text-white">
              <div v-if="warning && todo.status !== 'complete'">
            ...
            <div :class="{ 'animate-ping': warning && todo.status !== 'complete' }" class="absolute inline-flex h-full w-full rounded-full bg-brand-red opacity-75"></div>
          </div>
